...
SELECT atualizado, sum(quantidade), sum(minimo) FROM estoque GROUP BY atualizado; 
...
SELECT setor_id, sum(quantidade) AS total_pecas FROM estoque GROUP BY setor_id
...
SELECT setor_id, avg(realizado) FROM producao GROUP BY setor_id

-- A quantidade total de peças pedidas por fornecedor
SELECT fornecedor_id, sum(quantidade) FROM pedidos GROUP BY fornecedor_id
